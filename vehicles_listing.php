...
$results = mysqli_query($dbcon, "SELECT key_vehicles, fleet_number, vehicle_type, tag, year_made, model, max_seats, image_url, active_status FROM vehicles $sql_where ORDER BY " . cd($dbcon, $sql_order_by) . " " . cd($dbcon, $sql_order_by_seq) . " LIMIT " . cd($dbcon, $page_offset) . ", " . cd($dbcon, $items_per_page));
if ($results) {
	$table_rows = '';
...
<body id='page-listing' class='page_listing page_vehicles_listing'>
	...
	<section id='sub-menu'>
		...
		<div class='right-block'>
			&#9998;<a href='vehicles_save.php' target='overlay-iframe' onclick='overlayOpen();'>New</a>
		</div>
	</section>
