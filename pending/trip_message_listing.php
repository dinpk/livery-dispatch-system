...
$base_file_name = 'trips_messages_listing';
...
$count_results = mysqli_query($dbcon, "SELECT count(*) AS total_items FROM trips_messages $sql_where ");
...
$results = mysqli_query($dbcon, "SELECT key_trips_messages FROM trips_messages $sql_where ORDER BY " . cd($dbcon, $sql_order_by) . " " . cd($dbcon, $sql_order_by_seq) . " LIMIT " . cd($dbcon, $page_offset) . ", " . cd($dbcon, $items_per_page));
if ($results) {
	$table_rows = '';
	while ($row = mysqli_fetch_assoc($results)) {
		$record_id = $row['key_trips_messages'];
		$table_rows = $table_rows . '<tr>';
		$table_rows = $table_rows . "
		<td class='record-icons'>
		<a href='trip_message_save.php?trips_messagesid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>✎</a> 
		<a href='trip_message_view.php?trips_messagesid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>☷</a> 
		</td>";
		$table_rows = $table_rows . '</tr>';
	}
	$listing_html = "
...
<body id='page-listing' class='page_listing page_trips_messages_listing'>
	<?php include('php/_header.php'); ?>
	<section id='sub-menu'>
		<div class='left-block'>trips messages</div>
		<div class='right-block'>
			✢ <a href='trip_message_save.php' target='overlay-iframe' onclick='overlayOpen();'>New Message</a>
		</div>
	</section>
