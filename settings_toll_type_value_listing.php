...
$base_file_name = 'settings_toll_type_values_listing';
...
if (isset($_GET['search'])) {
	...
	if (empty($search)) {
		...
		$sql_where = "WHERE MATCH(toll_type) AGAINST('" . cd($dbcon, $search) . "')";
	}
}
...
$count_results = mysqli_query($dbcon, "SELECT count(*) AS total_items FROM settings_toll_type_values $sql_where ");
...
$results = mysqli_query($dbcon, "SELECT key_settings_toll_type_values, toll_type FROM settings_toll_type_values $sql_where ORDER BY " . cd($dbcon, $sql_order_by) . " " . cd($dbcon, $sql_order_by_seq) . " LIMIT " . cd($dbcon, $page_offset) . ", " . cd($dbcon, $items_per_page));
if ($results) {
	$table_rows = '';
	while ($row = mysqli_fetch_assoc($results)) {
		$record_id = $row['key_settings_toll_type_values'];
		$table_rows .= "
		<tr>
		<td>" . $row['toll_type'] . "</td>
		<td class='record-icons'>
		<a href='settings_toll_type_value_save.php?settings_toll_type_valuesid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>✎</a> 
		<a href='settings_toll_type_value_view.php?settings_toll_type_valuesid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>☷</a> 
		</td>
		</tr>
		";
	}
	$listing_html = "
		...
		<tr>
		<th><a href='$url" . $query_symbol . "sort_by=toll_type&sort_seq=$sql_order_by_seq'>Toll&nbsp;Type</a>" . (($sql_order_by == 'toll_type') ? $order_icon : '') . "</th>
...
<body id='page-listing' class='page_listing page_settings_toll_type_values_listing'>
	<?php include('php/_header.php'); ?>
	<section id='sub-menu'>
		<div class='left-block'><img src="images/icons/set_toll_types.png"> settings - toll types</div>
		<div class='right-block'>
			✢ <a href='settings_toll_type_value_save.php' target='overlay-iframe' onclick='overlayOpen();'>New Toll Type</a>
		</div>
	</section>
