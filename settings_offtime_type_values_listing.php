...
$base_file_name = 'settings_offtime_type_values_listing';
...
if (isset($_GET['search'])) {
	...
	if (empty($search)) {
		...
		$sql_where = "WHERE MATCH(offtime_type) AGAINST('" . cd($dbcon, $search) . "')";
	}
}
...
$count_results = mysqli_query($dbcon, "SELECT count(*) AS total_items FROM settings_offtime_type_values $sql_where ");
...
$results = mysqli_query($dbcon, "SELECT key_settings_offtime_type_values, offtime_type FROM settings_offtime_type_values $sql_where ORDER BY " . cd($dbcon, $sql_order_by) . " " . cd($dbcon, $sql_order_by_seq) . " LIMIT " . cd($dbcon, $page_offset) . ", " . cd($dbcon, $items_per_page));
if ($results) {
	$table_rows = '';
	while ($row = mysqli_fetch_assoc($results)) {
		$record_id = $row['key_settings_offtime_type_values'];
		$table_rows .= "
		<tr>
		<td>" . $row['offtime_type'] . "</td>
		<td class='record-icons'>
		<a href='settings_offtime_type_values_save.php?settings_offtime_type_valuesid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>✎</a> 
		<a href='settings_offtime_type_values_view.php?settings_offtime_type_valuesid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>☷</a> 
		</td>
		</tr>
		";
	}
	$listing_html = "
		...
		<tr>
		<th><a href='$url" . $query_symbol . "sort_by=offtime_type&sort_seq=$sql_order_by_seq'>Off-time&nbsp;Type</a>" . (($sql_order_by == 'offtime_type') ? $order_icon : '') . "</th>
...
<body id='page-listing' class='page_listing page_settings_offtime_type_values_listing'>
	<?php include('php/_header.php'); ?>
	<section id='sub-menu'>
		<div class='left-block'><img src="images/icons/set_off_time_types.png"> settings - offtime types</div>
		<div class='right-block'>
			✢ <a href='settings_offtime_type_values_save.php' target='overlay-iframe' onclick='overlayOpen();'>New Off-time Type</a>
		</div>
	</section>
