...
$base_file_name = 'settings_workshop_name_values_listing';
...
if (isset($_GET['search'])) {
	...
	if (empty($search)) {
		...
		$sql_where = "WHERE MATCH(workshop_name, city) AGAINST('" . cd($dbcon, $search) . "')";
	}
}
...
$count_results = mysqli_query($dbcon, "SELECT count(*) AS total_items FROM settings_workshop_name_values $sql_where ");
...
$results = mysqli_query($dbcon, "SELECT key_settings_workshop_name_values, workshop_name, contact_name, address1, address2, phone, email, city, state, zip_code FROM settings_workshop_name_values $sql_where ORDER BY " . cd($dbcon, $sql_order_by) . " " . cd($dbcon, $sql_order_by_seq) . " LIMIT " . cd($dbcon, $page_offset) . ", " . cd($dbcon, $items_per_page));
if ($results) {
	$table_rows = '';
	while ($row = mysqli_fetch_assoc($results)) {
		$record_id = $row['key_settings_workshop_name_values'];
		$table_rows .= "
		<tr>
		...
		<a href='settings_workshop_name_value_save.php?settings_workshop_name_valuesid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>✎</a> 
		<a href='settings_workshop_name_value_view.php?settings_workshop_name_valuesid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>☷</a> 
		</td>
		</tr>
		";
	}
	$listing_html = "
		...
		<th><a href='$url" . $query_symbol . "sort_by=contact_name&sort_seq=$sql_order_by_seq'>Contact</a>" . (($sql_order_by == 'contact_name') ? $order_icon : '') . "</th>
...
<body id='page-listing' class='page_listing page_settings_workshop_name_values_listing'>
	<?php include('php/_header.php'); ?>
	<section id='sub-menu'>
		<div class='left-block'><img src="images/icons/set_workshops.png"> settings - workshops</div>
		<div class='right-block'>
			✢ <a href='settings_workshop_name_value_save.php' target='overlay-iframe' onclick='overlayOpen();'>New Workshop</a>
		</div>
	</section>
