...
$base_file_name = 'settings_landmark_values_listing';
...
$count_results = mysqli_query($dbcon, "SELECT count(*) AS total_items FROM settings_landmark_values $sql_where ");
...
$results = mysqli_query($dbcon, "SELECT key_settings_landmark_values, category FROM settings_landmark_values $sql_where ORDER BY " . cd($dbcon, $sql_order_by) . " " . cd($dbcon, $sql_order_by_seq) . " LIMIT " . cd($dbcon, $page_offset) . ", " . cd($dbcon, $items_per_page));
if ($results) {
	$table_rows = '';
	while ($row = mysqli_fetch_assoc($results)) {
		$record_id = $row['key_settings_landmark_values'];
		$table_rows .= "
		<tr>
		<td>" . $row['category'] . "</td>
		<td class='record-icons'>
		<a href='settings_landmark_values_save.php?settings_landmark_valuesid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>✎</a> 
		<a href='settings_landmark_values_view.php?settings_landmark_valuesid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>☷</a> 
		</td>
		</tr>
		";
	}
...
<body id='page-listing' class='page_listing page_settings_landmark_values_listing'>
	<?php include('php/_header.php'); ?>
	<section id='sub-menu'>
		<div class='left-block'><img src="images/icons/set_landmarks.png"> settings - landmarks</div>
		<div class='right-block'>
			✢ <a href='settings_landmark_values_save.php' target='overlay-iframe' onclick='overlayOpen();'>New Landmark</a>
		</div>
	</section>
