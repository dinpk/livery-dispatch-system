...
if (isset($parent_id) && isset($record_id)) {
	$form_url_parameter = "?vehicles_maintenanceid=$record_id&vehiclesid=$parent_id"; // for record update		
} else if (isset($parent_id)) {
	$form_url_parameter = "?vehiclesid=$parent_id"; // for new record
}
// 'Save' button clicked
if (isset($_POST['save_submit'])) {
	$error = 0;
...
<body id='page-save' class='foreign'>
	...
	<form method='post' action='vehicles_maintenance_save.php<?php print $form_url_parameter; ?>'>
		<fieldset>

		<div>
		<label for='repair_date'>Repair date</label> <span class='red'> *</span><br>
		<?php if(isset($msg_repair_date)) print $msg_repair_date; ?>
		<input id='repair_date' name='repair_date' type='date' placeholder='yyyy-mm-dd' value='<?php if (isset($repair_date)) {print $repair_date;} ?>' required><br>
		</div>
		...
		<input id='warranty_expiration' name='warranty_expiration' type='date' placeholder='yyyy-mm-dd' value='<?php if (isset($warranty_expiration)) {print $warranty_expiration;} ?>'><br>
		</div>
