...
if (isset($_GET['search'])) {
	...
	if (empty($search)) {
		...
		$sql_where = "WHERE MATCH(log_type, action_performed) AGAINST('" . cd($dbcon, $search) . "')";
	}
}
...
$count_results = mysqli_query($dbcon, "SELECT count(*) AS total_items FROM logs $sql_where ");
...
$results = mysqli_query($dbcon, "SELECT key_logs, log_type, log_datetime, action_performed FROM logs $sql_where ORDER BY " . cd($dbcon, $sql_order_by) . " " . cd($dbcon, $sql_order_by_seq) . " LIMIT " . cd($dbcon, $page_offset) . ", " . cd($dbcon, $items_per_page));
if ($results) {
	$table_rows = '';
	while ($row = mysqli_fetch_assoc($results)) {
		$record_id = $row['key_logs'];
		$table_rows .= "
		<tr>
		<td>" . $row['log_type'] . "</td>
	...
	$listing_html = "
		...
		<tr>
		<th><a href='$url" . $query_symbol . "sort_by=log_type&sort_seq=$sql_order_by_seq'>Log&nbsp;Type</a>" . (($sql_order_by == 'log_type') ? $order_icon : '') . "</th>
		<th><a href='$url" . $query_symbol . "sort_by=log_datetime&sort_seq=$sql_order_by_seq'>Log&nbsp;Date&nbsp;Time</a>" . (($sql_order_by == 'log_datetime') ? $order_icon : '') . "</th>
		<th><a href='$url" . $query_symbol . "sort_by=action_performed&sort_seq=$sql_order_by_seq'>Action&nbsp;Performed</a>" . (($sql_order_by == 'action_performed') ? $order_icon : '') . "</th>
...
<body id='page-listing' class='page_listing page_logs_listing'>
	<?php include('php/_header.php'); ?>
	<section id='sub-menu'>
		<div class='left-block'>logs</div>
		<div class='right-block'>
			✢ <a href='logs_save.php' target='overlay-iframe' onclick='overlayOpen();'>New</a>
		</div>
	</section>
