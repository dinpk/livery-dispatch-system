...
$base_file_name = 'trips_payroll_settlement_listing';
...
$results = mysqli_query($dbcon, "SELECT key_trips, passenger_name, pickup_datetime, trip_status, driver_name, vehicle, zone_from, zone_to, total_trip_amount, pay_total_driver_amount, settled_checkbox, invoiced_checkbox FROM trips $sql_where ORDER BY " . cd($dbcon, $sql_order_by) . " " . cd($dbcon, $sql_order_by_seq) . " LIMIT " . cd($dbcon, $page_offset) . ", " . cd($dbcon, $items_per_page));
if ($results) {
	$table_rows = '';
	while ($row = mysqli_fetch_assoc($results)) {
		...
		<td class='right'>" . $row['pay_total_driver_amount'] . "</td>
	...
	$listing_html = "
		...
		<th><a href='$url" . $query_symbol . "sort_by=total_trip_amount&sort_seq=$sql_order_by_seq'>Trip&nbsp;Charges</a>" . (($sql_order_by == 'total_trip_amount') ? $order_icon : '') . "</th>
		<th><a href='$url" . $query_symbol . "sort_by=pay_total_driver_amount&sort_seq=$sql_order_by_seq'>Driver&nbsp;Charges</a>" . (($sql_order_by == 'pay_total_driver_amount') ? $order_icon : '') . "</th>
		<th><a href='$url" . $query_symbol . "sort_by=settled_checkbox&sort_seq=$sql_order_by_seq'>Set</a>" . (($sql_order_by == 'settled_checkbox') ? $order_icon : '') . "</th>
		<th><a href='$url" . $query_symbol . "sort_by=invoiced_checkbox&sort_seq=$sql_order_by_seq'>Inv</a>" . (($sql_order_by == 'invoiced_checkbox') ? $order_icon : '') . "</th>
