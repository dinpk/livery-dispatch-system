...
if (isset($_GET['search'])) {
	...
	if (empty($search)) {
		...
		$sql_where = "WHERE MATCH(company_name, first_name, last_name, address1, address2, city, state, zip_code, country, email) AGAINST('" . cd($dbcon, $search) . "')";
	}
}
...
$count_results = mysqli_query($dbcon, "SELECT count(*) AS total_items FROM customer_contacts $sql_where $sql_where_alt ");
...
$results = mysqli_query($dbcon, "SELECT key_customer_contacts, company_name, first_name, last_name, city, state, work_phone, work_phone_extension, email, active_status 
...
if ($results) {
	...
	while ($row = mysqli_fetch_assoc($results)) {
		$record_id = $row['key_customer_contacts'];
		$work_phone_extension = $row['work_phone_extension'];
		if (!empty($work_phone_extension)) $work_phone_extension = "($work_phone_extension)";
		...
		<td>" . $row['city'] . "</td>
		<td>" . $row['state'] . "</td>
		<td>" . $row['work_phone'] . " $work_phone_extension</td>
		<td>" . $row['email'] . "</td>
		...
		<td class='record-menus'>
			...
			<li><a href='customer_contacts_save.php?customer_contactsid=$record_id' target='overlay-iframe' onclick='overlayOpen();hide_record_menus();'>Edit</a></li>
			<li><a href='customer_contacts_view.php?customer_contactsid=$record_id' target='overlay-iframe' onclick='overlayOpen();hide_record_menus();'>View</a></li>
			<li><a href='trips_listing.php?customer_contactsid=$record_id' target='_blank' onclick='hide_record_menus();'>Trips</a></li>
			</ul>
		</td>
		</tr>";
	}
	$listing_html = "
			<table class='listing-table'>
				...
				<th><a href='$url" . $query_symbol . "sort_by=work_phone&sort_seq=$sql_order_by_seq'>Phone</a>" . (($sql_order_by == 'work_phone') ? $order_icon : '') . "</th>
...
<body id='page-listing' class='page_listing page_customer_contacts_listing'>
	...
	<section id='sub-menu'>
		...
		<div class='right-block'>
			✢ <a href='customer_contacts_save.php' target='overlay-iframe' onclick='overlayOpen();'>New Contact</a>
		</div>
	</section>
