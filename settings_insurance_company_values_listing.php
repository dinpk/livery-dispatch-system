...
$base_file_name = 'settings_insurance_company_values_listing';
...
$count_results = mysqli_query($dbcon, "SELECT count(*) AS total_items FROM settings_insurance_company_values $sql_where ");
...
$results = mysqli_query($dbcon, "SELECT key_settings_insurance_company_values, insurance_company FROM settings_insurance_company_values $sql_where ORDER BY " . cd($dbcon, $sql_order_by) . " " . cd($dbcon, $sql_order_by_seq) . " LIMIT " . cd($dbcon, $page_offset) . ", " . cd($dbcon, $items_per_page));
if ($results) {
	$table_rows = '';
	while ($row = mysqli_fetch_assoc($results)) {
		$record_id = $row['key_settings_insurance_company_values'];
		$table_rows .= "
		<tr>
		<td>" . $row['insurance_company'] . "</td>
		<td class='record-icons'>
		<a href='settings_insurance_company_values_save.php?settings_insurance_company_valuesid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>✎</a> 
		<a href='settings_insurance_company_values_view.php?settings_insurance_company_valuesid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>☷</a> 
		</td>
		</tr>
		";
	}
	$listing_html = "
		...
		<tr>
		<th><a href='$url" . $query_symbol . "sort_by=insurance_company&sort_seq=$sql_order_by_seq'>Insurance&nbsp;Company</a>" . (($sql_order_by == 'insurance_company') ? $order_icon : '') . "</th>
...
<body id='page-listing' class='page_listing page_settings_insurance_company_values_listing'>
	<?php include('php/_header.php'); ?>
	<section id='sub-menu'>
		<div class='left-block'><img src="images/icons/set_insurance_companies.png"> settings - insurance companies</div>
		<div class='right-block'>
			✢ <a href='settings_insurance_company_values_save.php' target='overlay-iframe' onclick='overlayOpen();'>New Insurance Company</a>
		</div>
	</section>
