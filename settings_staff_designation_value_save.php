...
if (isset($_POST['save_submit'])) {
    ...
    if ($error == 0) {
        ...
        if (isset($record_id) && $error != 1) { // update
            $results = mysqli_query($dbcon, "UPDATE `settings_staff_designation_values` SET `designation` = '" . sd($dbcon, $designation) . "' WHERE `key_settings_staff_designation_values` = $record_id");
            if ($results) {
            ...
            $results = mysqli_query($dbcon, "INSERT INTO `settings_staff_designation_values` (`designation`) VALUES ('" . sd($dbcon, $designation) . "')");
            if ($results) {
                $message = "<script>parent.location.reload(false);</script>";
            } else {
            ...
            <fieldset>
                <div>
                    ...
                    <input id='designation' name='designation' type='text'
                        value='<?php if (isset($designation)) {print $designation;} else { print '';} ?>' required><br>
                </div>
