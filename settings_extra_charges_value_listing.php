...
$base_file_name = 'settings_extra_charges_values_listing';
...
if (isset($_GET['search'])) {
	...
	if (empty($search)) {
		...
		$sql_where = "WHERE MATCH(category) AGAINST('" . cd($dbcon, $search) . "')";
	}
}
...
$count_results = mysqli_query($dbcon, "SELECT count(*) AS total_items FROM settings_extra_charges_values $sql_where ");
...
$results = mysqli_query($dbcon, "SELECT key_settings_extra_charges_values, category FROM settings_extra_charges_values $sql_where ORDER BY " . cd($dbcon, $sql_order_by) . " " . cd($dbcon, $sql_order_by_seq) . " LIMIT " . cd($dbcon, $page_offset) . ", " . cd($dbcon, $items_per_page));
if ($results) {
	$table_rows = '';
	while ($row = mysqli_fetch_assoc($results)) {
		$record_id = $row['key_settings_extra_charges_values'];
		$table_rows .= "
		<tr>
		<td>" . $row['category'] . "</td>
		<td class='record-icons'>
		<a href='settings_extra_charges_value_save.php?settings_extra_charges_valuesid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>✎</a> 
		<a href='settings_extra_charges_value_view.php?settings_extra_charges_valuesid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>☷</a> 
		</td>
		</tr>
		";
	}
...
<html>
<head>
	<title>SETTINGS - EXTRA CHARGES VALUES</title>
	<?php include('php/_head.php'); ?>
</head>
<body id='page-listing' class='page_listing page_settings_extra_charges_values_listing'>
	<?php include('php/_header.php'); ?>
	<section id='sub-menu'>
		<div class='left-block'><img src="images/icons/set_extra_charges.png"> settings - extra charges</div>
		<div class='right-block'>
			✢ <a href='settings_extra_charges_value_save.php' target='overlay-iframe' onclick='overlayOpen();'>New Extra Charge</a>
		</div>
	</section>
