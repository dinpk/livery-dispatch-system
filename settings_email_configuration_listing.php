...
$base_file_name = 'settings_email_configuration_listing';
...
$count_results = mysqli_query($dbcon, "SELECT count(*) AS total_items FROM settings_email_configuration $sql_where ");
...
$results = mysqli_query($dbcon, "SELECT key_settings_email_configuration, sender_email, sender_password, reply_to_email, copy_to_email, smtp_address FROM settings_email_configuration $sql_where ORDER BY " . cd($dbcon, $sql_order_by) . " " . cd($dbcon, $sql_order_by_seq) . " LIMIT " . cd($dbcon, $page_offset) . ", " . cd($dbcon, $items_per_page));
if ($results) {
	$table_rows = '';
	while ($row = mysqli_fetch_assoc($results)) {
		$record_id = $row['key_settings_email_configuration'];
		$table_rows .= "
		<tr>
		<td>" . $row['sender_email'] . "</td>
		<td>" . $row['sender_password'] . "</td>
		<td>" . $row['reply_to_email'] . "</td>
		<td>" . $row['copy_to_email'] . "</td>
		<td>" . $row['smtp_address'] . "</td>
		<td class='record-icons'>
		<a href='settings_email_configuration_save.php?settings_email_configurationid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>✎</a> 
		<a href='settings_email_configuration_view.php?settings_email_configurationid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>☷</a> 
		</td>
		</tr>
		";
	}
	$listing_html = "
		...
		<tr>
		<th><a href='$url" . $query_symbol . "sort_by=sender_email&sort_seq=$sql_order_by_seq'>Sender&nbsp;Email</a>" . (($sql_order_by == 'sender_email') ? $order_icon : '') . "</th>
		<th><a href='$url" . $query_symbol . "sort_by=sender_password&sort_seq=$sql_order_by_seq'>Sender&nbsp;Password</a>" . (($sql_order_by == 'sender_password') ? $order_icon : '') . "</th>
		<th><a href='$url" . $query_symbol . "sort_by=reply_to_email&sort_seq=$sql_order_by_seq'>Reply&nbsp;To&nbsp;Email</a>" . (($sql_order_by == 'reply_to_email') ? $order_icon : '') . "</th>
		<th><a href='$url" . $query_symbol . "sort_by=copy_to_email&sort_seq=$sql_order_by_seq'>Copy&nbsp;To&nbsp;Email</a>" . (($sql_order_by == 'copy_to_email') ? $order_icon : '') . "</th>
		<th><a href='$url" . $query_symbol . "sort_by=smtp_address&sort_seq=$sql_order_by_seq'>Smtp</a>" . (($sql_order_by == 'smtp_address') ? $order_icon : '') . "</th>
...
<body id='page-listing' class='page_listing page_settings_email_configuration_listing'>
	<?php include('php/_header.php'); ?>
	<section id='sub-menu'>
		<div class='left-block'><img src="images/icons/set_email_configuration.png"> email configuration</div>
		<div class='right-block'>
			✢ <a href='settings_email_configuration_save.php' target='overlay-iframe' onclick='overlayOpen();'>New Email Configuration</a>
		</div>
	</section>
