...
if (isset($_GET['search'])) {
	...
	if (empty($search)) {
		...
		$sql_where = "WHERE MATCH(payment_card_type) AGAINST('" . cd($dbcon, $search) . "')";
	}
}
...
$count_results = mysqli_query($dbcon, "SELECT count(*) AS total_items FROM settings_payment_card_type_values $sql_where ");
...
$results = mysqli_query($dbcon, "SELECT key_settings_payment_card_type_values, payment_card_type FROM settings_payment_card_type_values $sql_where ORDER BY " . cd($dbcon, $sql_order_by) . " " . cd($dbcon, $sql_order_by_seq) . " LIMIT " . cd($dbcon, $page_offset) . ", " . cd($dbcon, $items_per_page));
if ($results) {
	$table_rows = '';
	while ($row = mysqli_fetch_assoc($results)) {
		$record_id = $row['key_settings_payment_card_type_values'];
		$table_rows .= "
		<tr>
		<td>" . $row['payment_card_type'] . "</td>
		<td class='record-icons'>
		<a href='settings_payment_card_type_values_save.php?settings_payment_card_type_valuesid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>✎</a> 
		<a href='settings_payment_card_type_values_view.php?settings_payment_card_type_valuesid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>☷</a> 
		</td>
		</tr>
		";
	}
	$listing_html = "
		...
		<tr>
		<th><a href='$url" . $query_symbol . "sort_by=payment_card_type&sort_seq=$sql_order_by_seq'>Payment&nbsp;Card&nbsp;Type</a>" . (($sql_order_by == 'payment_card_type') ? $order_icon : '') . "</th>
...
<body id='page-listing' class='page_listing page_settings_payment_card_type_values_listing'>
	...
	<section id='sub-menu'>
		...
		<div class='right-block'>
			✢ <a href='settings_payment_card_type_values_save.php' target='overlay-iframe' onclick='overlayOpen();'>New Payment Card Type</a>
		</div>
	</section>
