...
if (isset($_POST['save_submit'])) {
	...
	if ($error == 0) {
		if (isset($record_id) && $error != 1) { // update
			$results = mysqli_query($dbcon, "UPDATE settings_landmark_values SET 
			category = '" . sd($dbcon, $category) . "'
				WHERE key_settings_landmark_values = $record_id");
			if ($results) {
			...
			$results = mysqli_query($dbcon, "INSERT INTO settings_landmark_values (category) VALUES ('" . sd($dbcon, $category) . "')");
			if ($results) {
				$message = "<script>parent.location.reload(false);</script>";
			} else {
