...
// used by customer_passenger_listing
...
<head>
    ...
    <script>
    function set_passenger_id_for_select_address(
    recordid) { // called from (1) trips_select_customer_passengers (2) bottom of this page
        document.getElementById("select_customer_address_link").href =
            "trip_select_customer_address_book.php?recordid=" + recordid;
    }
    </script>
</head>
<body id='page-save'>
    <section id='sub-menu'>
        <div class='left-block'>
			trip 
			<?php 
				if (isset($key_trips)) print '#' . $key_trips; 
				if (isset($extra_title)) print $extra_title; 
			?>
		</div>
    ...
    <main>
        ...
        <form method='post'>
            <fieldset id="trips_fieldset1"> <input id='key_customer_passengers' name='key_customer_passengers'
                    type='hidden'
                    value='<?php if (isset($key_customer_passengers)) {print $key_customer_passengers;} else {print '0';} ?>'>
                <input id='key_customer_contacts' name='key_customer_contacts' type='hidden'
                    value='<?php if (isset($key_customer_contacts)) {print $key_customer_contacts;} else {print '0';} ?>'>
                <input id='key_drivers' name='key_drivers' type='hidden'
                    value='<?php if (isset($key_drivers)) {print $key_drivers;} else {print '0';} ?>'>
                <input id='key_vehicles' name='key_vehicles' type='hidden'
                    value='<?php if (isset($key_vehicles)) {print $key_vehicles;} else {print '0';} ?>'>
                <input id='key_settings_airline_values' name='key_settings_airline_values' type='hidden'
                    value='<?php if (isset($key_settings_airline_values)) {print $key_settings_airline_values;} else {print '0';} ?>'>
                <input id='key_rates_zones' name='key_rates_zones' type='hidden'
                    value='<?php if (isset($key_rates_zones)) {print $key_rates_zones;} else {print '0';} ?>'>
                <div>
                    ...
                    <input id='reference_number' name='reference_number' type='text'
                        value='<?php if (isset($reference_number)) {print $reference_number;} else { print '';} ?>'><br>
                </div>
                <div>
                    <label for='passenger_name'>Passenger name</label> <span class='red'> *</span>
                    <small>
                        <a href='trip_select_customer_passenger.php' target='overlay-iframe2'
                            onclick='overlayOpen2();'>Select</a> &nbsp;
                        <a href='#'
                            onclick='unselectKeyValue("key_customer_passengers","passenger_name");return false;'>x</a>
                    </small><br>
                    <?php if(isset($msg_passenger_name)) print $msg_passenger_name; ?>
                    <input id='passenger_name' name='passenger_name' type='text'
                        value='<?php if (isset($passenger_name)) {print $passenger_name;} else { print '';} ?>' required
                        readonly><br>
                </div>
                <div>
                    <label for='total_passengers'>Total passengers</label>
                    <?php if(isset($msg_total_passengers)) print $msg_total_passengers; ?>
                    <input id='total_passengers' name='total_passengers' type='number'
                        value='<?php if (isset($total_passengers)) {print $total_passengers;} else { print '1';} ?>'><br>
                </div>
                <div>
                    <label for='reserved_by'>Reserved by</label>
                    <small>
                        <a href='trip_select_customer_contact.php' target='overlay-iframe2'
                            onclick='overlayOpen2();'>Select</a> &nbsp;
                        <a href='#'
                            onclick='unselectKeyValue("key_customer_contacts","reserved_by");return false;'>x</a>
                    </small><br>
                    <?php if(isset($msg_reserved_by)) print $msg_reserved_by; ?>
                    <input id='reserved_by' name='reserved_by' type='text'
                        value='<?php if (isset($reserved_by)) {print $reserved_by;} else { print '';} ?>' readonly><br>
                </div>
                <div>
                    <label for='pickup_date'>Pickup date</label><br>
                    <?php if(isset($msg_pickup_date)) print $msg_pickup_date; ?>
                    <input id='pickup_date' name='pickup_date' type='date' placeholder='yyyy-mm-dd'
                        value='<?php if (isset($pickup_date)) {print $pickup_date;} ?>'><br>
                </div>
                <div>
                    <label for='pickup_time'>Pickup time</label><br>
                    <?php if(isset($msg_pickup_time)) print $msg_pickup_time; ?>
                    <input id='pickup_time' name='pickup_time' type='time' placeholder='hh:mm (24-hour)'
                        value='<?php if (isset($pickup_time)) {print $pickup_time;} ?>'><br>
                </div>
                <div>
                    <label for='dropoff_date'>Dropoff date</label><br>
                    <?php if(isset($msg_dropoff_date)) print $msg_dropoff_date; ?>
                    <input id='dropoff_date' name='dropoff_date' type='date' placeholder='yyyy-mm-dd'
                        value='<?php if (isset($dropoff_date)) {print $dropoff_date;} ?>'><br>
                </div>
                <div>
                    <label for='dropoff_time'>Dropoff time</label><br>
                    <?php if(isset($msg_dropoff_time)) print $msg_dropoff_time; ?>
                    <input id='dropoff_time' name='dropoff_time' type='time' placeholder='hh:mm (24-hour)'
                        value='<?php if (isset($dropoff_time)) {print $dropoff_time;} ?>'><br>
                </div>
                <div>
            ...
            <fieldset id="trips_fieldset2">
                <div>
                    ...
                    <small>
                        <a href='trip_select_driver.php' target='overlay-iframe2' onclick='overlayOpen2();'>Select</a>
                        &nbsp;
                    ...
                    <input id='driver_name' name='driver_name' type='text'
                        value='<?php if (isset($driver_name)) {print $driver_name;} else { print '';} ?>' readonly><br>
                </div>
                <div>
                    <label for='vehicle'>Vehicle</label>
                    <small>
                        <a href='trip_select_vehicle.php?ratezoneid=<?php print $key_rates_zones; ?>'
                            target='overlay-iframe2' onclick='overlayOpen2();'>Select</a> &nbsp;
                    ...
                    <input id='vehicle' name='vehicle' type='text'
                        value='<?php if (isset($vehicle)) {print $vehicle;} else { print '';} ?>' readonly><br>
                </div>
                <div>
                    <label for='airline'>Airline</label>
                    <small>
                        <a href='trip_select_airline.php' target='overlay-iframe2' onclick='overlayOpen2();'>Select</a>
                        &nbsp;
                        <a href='#'
                            onclick='unselectKeyValue("key_settings_airline_values","airline");return false;'>x</a>
                    </small><br>
                    <?php if(isset($msg_airline)) print $msg_airline; ?>
                    <input id='airline' name='airline' type='text'
                        value='<?php if (isset($airline)) {print $airline;} else { print '';} ?>' readonly><br>
                </div>
                <div>
                    ...
                    <input id='flight_number' name='flight_number' type='text'
                        value='<?php if (isset($flight_number)) {print $flight_number;} else { print '';} ?>'><br>
                </div>
                <div>
                    <label for='zone_from'>Zone from</label>
                    <small>
                        <a href='trip_select_rate_zone.php' target='overlay-iframe2'
                            onclick='overlayOpen2();'>Select</a> &nbsp;
                        <a href='#'
                            onclick='unselectKeyValue("key_rates_zones","zone_from");unselectKeyValue("key_rates_zones","zone_to");return false;'>x</a>
                    </small><br>
                    <?php if(isset($msg_zone_from)) print $msg_zone_from; ?>
                    <input id='zone_from' name='zone_from' type='text'
                        value='<?php if (isset($zone_from)) {print $zone_from;} else { print '';} ?>' readonly><br>
                </div>
                <div>
                    <label for='zone_to'>Zone to</label>
                    <?php if(isset($msg_zone_to)) print $msg_zone_to; ?>
                    <input id='zone_to' name='zone_to' type='text'
                        value='<?php if (isset($zone_to)) {print $zone_to;} else { print '';} ?>' readonly><br>
                </div>
            </fieldset>
            <fieldset id="trips_fieldset3">
                <div>
                    <p>
                        <small>
                            <a href='trip_select_landmark.php' target='overlay-iframe2'
                                onclick='overlayOpen2();'>Landmarks</a> &nbsp;
                            <!-- a function in <head> sets href for this link which is called from trip_select_customer_passenger.php -->
                            <a id='select_customer_address_link' href='trip_select_customer_address_book.php'
                                target='overlay-iframe2' onclick='overlayOpen2();'>Addressbook</a>
                        </small>
                    </p>
                    <label for='routing_from'>Routing from</label> <span class='red'> *</span>
                    <?php if(isset($msg_routing_from)) print $msg_routing_from; ?>
                    <textarea id='routing_from' name='routing_from'
                        required><?php if (isset($routing_from)) print $routing_from; ?></textarea><br>
                </div>
                <div>
                    <label for='routing_to'>Routing to</label> <span class='red'> *</span>
                    <?php if(isset($msg_routing_to)) print $msg_routing_to; ?>
                    <textarea id='routing_to' name='routing_to'
                        required><?php if (isset($routing_to)) print $routing_to; ?></textarea><br>
                </div>
                <div>
                    <label for='routing_notes'>Routing notes</label>
                    <?php if(isset($msg_routing_notes)) print $msg_routing_notes; ?>
                    <textarea id='routing_notes'
                        name='routing_notes'><?php if (isset($routing_notes)) print $routing_notes; ?></textarea><br>
                </div>
                <div>
                    <label for='dispatcher_notes'>Dispatcher notes</label>
                    <?php if(isset($msg_dispatcher_notes)) print $msg_dispatcher_notes; ?>
                    <textarea id='dispatcher_notes'
                        name='dispatcher_notes'><?php if (isset($dispatcher_notes)) print $dispatcher_notes; ?></textarea><br>
                </div>
            </fieldset>
