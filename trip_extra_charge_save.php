...
if (isset($_GET['tripextrachargeid'])) {
	...
	if (!isset($_POST['save_submit'])) {		$results = mysqli_query($dbcon, "SELECT * FROM trip_extra_charges WHERE key_trip_extra_charges = $record_id  AND key_trips = $parent_id");
		if ($row = mysqli_fetch_assoc($results)) {
            ...
            <fieldset>
                <div>
                    ...
                    <select id='category' name='category' required>
                        <?php 
						$options = '';		$results = mysqli_query($dbcon, 'SELECT category FROM settings_extra_charges_values');
                ...
                <div>
                    ...
                    <input id='amount' name='amount' type='number' step='any'
                        value='<?php if (isset($amount)) {print $amount;} else {print '0';} ?>' required><br>
                </div>
                <div>
