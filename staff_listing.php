...
if (isset($_GET['search'])) {
	...
	if (empty($search)) {
		...
		$sql_where = "WHERE MATCH(designation, first_name, last_name) AGAINST('" . cd($dbcon, $search) . "')";
	}
}
...
if ($results) {
	...
	while ($row = mysqli_fetch_assoc($results)) {
		...
		<a href='staff_save.php?staffid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>✎</a> 
		<a href='staff_view.php?staffid=$record_id' target='overlay-iframe' onclick='overlayOpen();'>☷</a> 
		</td>
		</tr>
		";
	}
	$listing_html = "
		...
		<tr>
		<th><a href='$url" . $query_symbol . "sort_by=username&sort_seq=$sql_order_by_seq'>Username</a>" . (($sql_order_by == 'username') ? $order_icon : '') . "</th>
