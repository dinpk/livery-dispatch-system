...
$base_file_name = 'drivers_listing';
...
if (isset($_GET['search'])) {
	...
	if (empty($search)) {
		...
		$sql_where = "WHERE MATCH(first_name, last_name, contract_type, city, state, zip_code, fleet_number) AGAINST('" . cd($dbcon, $search) . "')";
	}
}
...
if ($results) {
	...
	while ($row = mysqli_fetch_assoc($results)) {
		...
		<li><a href='driver_save.php?driversid=$record_id' target='overlay-iframe' onclick='overlayOpen();hide_record_menus();'>Edit</a></li>
		<li><a href='driver_view.php?driversid=$record_id' target='overlay-iframe' onclick='overlayOpen();hide_record_menus();'>View</a></li>
		<li><a href='trip_listing.php?driversid=$record_id' target='_blank' onclick='hide_record_menus();'>Trips</a></li>
		<li><a href='driver_payroll_listing.php?driversid=$record_id' target='_blank' onclick='hide_record_menus();'>Payroll</a></li>
		</ul>
		</td>
		</tr>
		";
...
<body id='page-listing' class='page_listing page_drivers_listing'>
	<?php include('php/_header.php'); ?>
	<section id='sub-menu'>
		<div class='left-block'><img src="images/icons/nav_drivers.png"> drivers</div>
		<div class='right-block'>
			✢ <a href='driver_save.php' target='overlay-iframe' onclick='overlayOpen();'>New Driver</a>
		</div>
	</section>
